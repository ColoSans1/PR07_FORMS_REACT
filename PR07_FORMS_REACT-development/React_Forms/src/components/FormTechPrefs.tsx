/* Componente reutilizable para el formulario de preferencias en tecnología */
...
interface TechPrefsData {
  [key: string]: string | number | string[];
}

interface FormTechPrefsProps {
  onSubmit: (data: TechPrefsData) => void;
}

/* Componente FormTechPrefs para gestionar dinámicamente el formulario de tecnología */
const FormTechPrefs: React.FC<FormTechPrefsProps> = ({ onSubmit }) => {
  const [formData, setFormData] = useState<TechPrefsData>({});
  ...
  useEffect(() => {
    fetch('/cuestionario.json')
      ...
      .then(data => {
        ...
        const techForm = data.formularios.find((f: Formulario) => f.id === 'tech');
        setCurrentForm(techForm || null);
      })
  ...
  const validateField = (campo: Campo, value: string | string[]): string => {
    ...
    if (campo.id === 'experienceYears' && typeof value === 'string') {
      ...
      if (isNaN(years) || years < (campo.validacion?.min_edad || 0))
        error = `Los años de experiencia deben ser mayor a ${campo.validacion?.min_edad || 0}`;
    }
    return error;
  };
